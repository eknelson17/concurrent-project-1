 ...
 * 
 * Threaded Project Manager Goes to work like employees, but also has an extra
 * morning meeting, executive meetings and can answer questions Cannot ask
 * questions
 */
public class ProjectManager extends Employee {
	...
	public ProjectManager(CountDownLatch startcdl, CountDownLatch lastMeeting,
			CountDownLatch lastMeetingOver, CountDownLatch firstMeeting) {
		super(-1, -1, startcdl, lastMeeting, lastMeetingOver);
		morningMeeting = firstMeeting;
	}

	/**
	 * Runs the PM. Shouldn't be called, use employee.start() instead. PM
	 * arrives at 8:00, goes to meeting with Team Leads, does work, goes to an
	 * exec meeting, goes to lunch, does more work, goes to a second exec
	 * meeting, goes to a final meeting, works more, then goes home.
	 */
	@Override
	public void run() {
	...
	/**
	 * Answers a question for a Team Lead Causes the current thread to wait for
	 * 10 simulation minutes.
	 */
